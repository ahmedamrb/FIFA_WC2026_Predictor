...
**Tasks**
- [x] Create a new private GitHub repository named `FIFA_WC2026_Predictor`.
- [x] Clone the repository to your local machine.
- [x] Create the full directory structure as shown in the Project Structure above — all folders and all `__init__.py` files.
- [x] Create an `outputs/plots/` directory for all charts and plots generated by scripts.
- [x] Create a `requirements.txt` file listing all libraries from the PRD tech stack: `pandas`, `numpy`, `scikit-learn`, `xgboost`, `optuna`, `statsmodels`, `streamlit`, `plotly`, `requests`, `httpx`, `beautifulsoup4`, `lxml`, `joblib`, `python-dotenv`, `pyarrow`, `kaleido`.
- [x] Create a `.gitignore` that excludes: `data/raw/`, `data/processed/`, `models/*.pkl`, `__pycache__/`, `.env`, `*.pyc`, `venv/`, `outputs/plots/`.
- [x] Create a `.env` file in the project root with a placeholder: `FD_API_KEY=your_key_here`.
- [x] Create a `README.md` with project title, one-paragraph description, and a placeholder "Setup" and "Usage" section.
- [x] Create a virtual environment locally (`venv/`) and install all packages from `requirements.txt`.
- [x] Make an initial commit with the full empty structure.

**Verification Checklist**
- [x] `git status` shows a clean working tree after the initial commit.
- [x] All directories from the Project Structure exist when inspected locally.
- [x] Running `python -c "import pandas, numpy, sklearn, xgboost, optuna, streamlit, plotly, joblib, dotenv, pyarrow"` produces no import errors.
- [x] Placing a dummy file inside `data/raw/` and running `git status` confirms it is ignored.
- [x] `.env` does not appear in `git status`.
- [x] `requirements.txt` is present and committed at the repo root.
...
- [ ] Navigate to the dataset `martj42/international-football-results-from-1872-to-2017` on Kaggle.
...
- [ ] Shape output printed: rows > 45,000, columns = 9.
...
- [ ] Four shapes printed to stdout matching expected sizes (results > 45,000 rows, rankings > 10,000, fixtures ≥ 104).
